...
public class Save implements CommandExecutor {
    ...
    public GameValues saveGame(int num) throws SQLException {
        ...
        if (gameValues == null){
            gameValues = new GameValues(num, GameValues.getP1().getUniqueId().toString(),GameValues.getP2().getUniqueId().toString(),
                    boardToString(accept.getBoard()),GameValues.getGameTurn());
